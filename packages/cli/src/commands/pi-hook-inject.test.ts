...
function installPiInjectEnv(): { pluginLogPath: string; cleanup: () => void } {
	const originalPluginLogPath = process.env.CODEMEM_PLUGIN_LOG_PATH;
	const tempDir = mkdtempSync(join(tmpdir(), "codemem-cli-pi-inject-"));
	const pluginLogPath = join(tempDir, "plugin.log");
...
describe("pi-hook-inject command", () => {
	let env: ReturnType<typeof installPiInjectEnv>;
	beforeEach(() => {
		env = installPiInjectEnv();
	});
	afterEach(() => {
		env.cleanup();
	});
...
describe("pi-hook-inject disable paths", () => {
	let env: ReturnType<typeof installPiInjectEnv>;
	beforeEach(() => {
		env = installPiInjectEnv();
	});
...
describe("pi-hook-inject formatting", () => {
	let env: ReturnType<typeof installPiInjectEnv>;
	beforeEach(() => {
		env = installPiInjectEnv();
	});
