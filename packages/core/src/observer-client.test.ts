...
describe("ObserverClient", () => {
	const savedHome: { HOME?: string; PI_CODING_AGENT_DIR?: string } = {};
	let isolatedHome: string | undefined;

	beforeEach(() => {
		savedHome.HOME = process.env.HOME;
		savedHome.PI_CODING_AGENT_DIR = process.env.PI_CODING_AGENT_DIR;
		isolatedHome = mkdtempSync(join(tmpdir(), "codemem-obs-client-home-"));
		process.env.HOME = isolatedHome;
	...
	afterEach(() => {
		...
		if (savedHome.PI_CODING_AGENT_DIR === undefined) delete process.env.PI_CODING_AGENT_DIR;
		else process.env.PI_CODING_AGENT_DIR = savedHome.PI_CODING_AGENT_DIR;
		if (isolatedHome) rmSync(isolatedHome, { recursive: true, force: true });
		isolatedHome = undefined;
	});
...
describe("ObserverClient — pi-derived auth (D8)", () => {
	...
	it("does not send an unrelated pi key to an explicit Anthropic observer", () => {
		writePiApiKeyFixture({ provider: "acme" });
